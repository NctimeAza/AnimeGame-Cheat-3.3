...
std::optional<ImageLoader::GIFData*> ImageLoader::GetGif(const std::string& imageName, const char* type)
{
	...
	//bool loadResult = ResourceLoader::LoadEx(111, RT_RCDATA, pDestination, size);
	bool loadResult = ResourceLoader::LoadEx(imageName.c_str(), type, pDestination, size);
	if (!loadResult)
	{
