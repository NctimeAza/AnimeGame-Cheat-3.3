...
namespace cheat::feature
{
	...
	void RSAPatch::DrawMain()
	{
		...
		ConfigWidget(_TR("Custom Public Key"), f_PublicKey, _TR("Dress Texture.\n" \
			"Example path: C:\\Dress.png"));
		ConfigWidget(_TR("Custom Private Key"), f_PrivateKey, _TR("Glider Texture.\n" \
	...
	static void RSA_FromXmlString_Hook(app::RSA *__this, app::String *xmlString, MethodInfo *method)
	{
		...
		if (rsaPatch.f_Enabled->enabled())
		{
			...
			LOG_WARNING("Get RSA Key:");
			LOG_WARNING(key.c_str());
			...
			{
				LOG_WARNING("Patching Public Key");
				LOG_WARNING(key.c_str());
				memcpy(xmlString, public_key, public_key->fields.length * 2);
			}

			// Private key: no need to use on grasscutter
			if (rsaPatch.f_PrivateKey->enabled())
			{
				...
				{
					LOG_WARNING("Patching Private Key");
					LOG_WARNING(key.c_str());
